class Solution:
    ...
    def numDistinct(self, s: str, t: str) -> int:
        """
        Time: O(n * m) # iをjの文字列として合致するかは必ず実行される
        Space: O(n * m) # 結果をキャッシュする際に必要な空間のためそれぞれを依存する状態数は時間と同じ
        """        
        memo = {}
        def dfs(i, j): # i,j  is index of s, t
            if j == len(t):
                return 1
            if i == len(s):
                return 0
            ...
            if s[i] == t[j]:
                memo[(i, j)] = dfs(i + 1, j + 1) + dfs(i + 1, j) # 1項目はiをjと合致する文字として使った時、2項目はiをjと合致する文字として使わなかった時
            else:
                memo[(i, j)] = dfs(i + 1, j)
