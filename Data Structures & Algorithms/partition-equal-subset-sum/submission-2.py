class Solution:
    ...
    def canPartition(self, nums: List[int]) -> bool:
        def dfs(total1, total2, index, memo):
            if (total1, total2, index) in memo:
                return memo[(total1, total2, index)]
            if index == len(nums):
                return total1 == total2
            
            memo[(total1, total2, index)] = memo[(total2, total1)] = dfs(total1 + nums[index], total2, index + 1, memo) or dfs(total1, total2 + nums[index], index + 1, memo)
            return memo[(total1, total2, index)]
