...
class Solution:
    """
    Task: Get Maximum area of water tha can be trapped between bars.

    First, We need to think about how to calculated aret trapped.

    I think the logic like this, we gonna use the 2 pointers like left&right.

    left: the height of left is bigger than the height of right. 
    right: samller than left basically

    if the height of the left is samller than the height of the rihgt.

    we need to reset the total trapped area. left is updated to right. right is just incremented.
    Before resetting the trapped area, we need to compare the max trapped area.

    The one trapped area is dermined by adding up the differ between the height of right and height of left bars.
